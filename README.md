...
- Также немного "раскрашена" переведена на utf-8 веб-админка
- Добавлена возможность получать статистику трафика в формате JSON по правилам count[in,out,all]


### Логика работы
При после израсходования пользователем своего лимита в countin/countout/countall, политики bandlim определяют пользователя `USER`(реальный логин пользователя), как виртуального `~USER`.  
Это позволяет включать другие ограничения скорости, когда пользователь израсходовал пакет трафика.

Если политики для `~USER` нет, то ограничений не будет, но будут сообщения в логе *Quota exceeded, applying bandlim from ~USER*.

Создавать пользователя `~USER` не нужно, он виртуальный, достаточно указать его в политике.

У 3proxy есть архитектурный баг: если пользователь израсходовал трафик в рамках уже открытого соединения (например, при скачивании большого файла), то соединение будет разорвано не сразу, а только когда 3proxy обнаружит превышение лимита (как правило, это происходит с небольшой задержкой в 10–30 МБ).  
